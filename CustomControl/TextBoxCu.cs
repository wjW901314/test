...
namespace CustomControl
{
    public class TextBoxCu : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            {
                g.FillPath(sb, RenderHelper.CreateRoundedRectanglePath(new Rectangle(0, 0, this.Width - 1, this.Height - 1), 5));
            }
            using (LinearGradientBrush lgb = new LinearGradientBrush(
               new Point(0, 1), new Point(0, this.Height),
               Color.LightGray, Color.White))
            {
                g.DrawPath(new Pen(lgb), RenderHelper.CreateRoundedRectanglePath(new Rectangle(0, 1, this.Width - 1, this.Height - 2), 5));
            }
            using (LinearGradientBrush lgb = new LinearGradientBrush(new Point(0, 0), new Point(0, this.Height - 1),
               m_bFocus ? Color.DodgerBlue : Color.Gray, Color.FromArgb(20, 0, 0, 0)))
            {
                g.DrawPath(new Pen(lgb), RenderHelper.CreateRoundedRectanglePath(new Rectangle(0, 0, this.Width - 1, this.Height - 2), 5));
            }
            base.OnPaint(e);
        }
    }
    ...
    {
        public static GraphicsPath CreateRoundedRectanglePath(Rectangle rec, int radius)
        {
            GraphicsPath path = new GraphicsPath();
            path.StartFigure();
            path.AddArc(rec.Left, rec.Top, radius * 2, radius * 2, 180, 90);
            path.AddArc(rec.Right - radius * 2, rec.Top, radius * 2, radius * 2, 270, 90);
            path.AddArc(rec.Right - radius * 2, rec.Bottom - radius * 2, radius * 2, radius * 2, 0, 90);
            path.AddArc(rec.Left, rec.Bottom - radius * 2, radius * 2, radius * 2, 90, 90);
            path.CloseFigure();
            return path;
        }
    }
